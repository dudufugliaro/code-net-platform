...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        const loginRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/token/`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        <div className="relative z-10">
          <div className="flex justify-center mb-6">
            <div className="w-16 h-16 bg-gradient-to-br from-violet-100 to-fuchsia-100 dark:from-violet-900/40 dark:to-fuchsia-900/40 rounded-2xl flex items-center justify-center shadow-inner">
              <UserPlus className="w-8 h-8 text-violet-600 dark:text-violet-400" />
            </div>
          </div>
